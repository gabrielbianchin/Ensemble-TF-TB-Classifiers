...
def validate(b2):
  model = createNN(b2=b2)
  es = EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='min', restore_best_weights=True)
  lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, mode='min', verbose=1)
  model.fit([X_train[:, :, :21], X_train[:, :, 21:]], y_train, epochs=50, batch_size=32, callbacks=[es, lr], validation_data=([X_val[:, :, :21], X_val[:, :, 21:]], y_val))
...
for b2 in [3, 4, 5, 6, 7]:
  ...
  np.save('/content/drive/My Drive/IV4B/cb6133-IV4B-' + str(b2) +'-val.npy', y_pred)
  ...
  y_pred = np.array(y_pred)
  np.save('/content/drive/My Drive/IV4B/cb6133-IV4B-' + str(b2) +'-test.npy', y_pred)